...
public:
    TCPSessionOfServer(boost::asio::io_service &TMIOService, boost::asio::io_service& io_service, 
        ...
        const unsigned int uiSSLEnabled = false, boost::shared_ptr<boost::asio::ssl::context> pSSLContext = boost::shared_ptr<boost::asio::ssl::context>());
    ...
    boost::shared_ptr<boost::asio::ssl::stream<boost::asio::ip::tcp::socket> > &GetSSLSocket();
...
private:
    ...
    boost::shared_ptr<boost::asio::ssl::stream<boost::asio::ip::tcp::socket> > m_pSSLSocket;
...
{
public:
	TCPServer(const unsigned short port, const unsigned int uiSSLEnabled = false);
    ...
    struct SSLServerInfo 
    {
        std::string strSSLCert;
        std::string strSSLCertKey;
        std::string strSSLCertDh;
        std::string strSSLCertPassword;
    };

    void SetSSLServerInfo(const SSLServerInfo &sslinfo)
    {
        m_SSLServerInfo = sslinfo;
    }
...
private:
    ...
    {        
        return m_SSLServerInfo.strSSLCertPassword.empty() ? "test" : m_SSLServerInfo.strSSLCertPassword;
    }

    void HandleSSLShakehand(boost::shared_ptr<TCPSessionOfServer> pSession, const boost::system::error_code& error);
...
private:
    ...
    unsigned int m_uiSSLEnabled;
    boost::shared_ptr<boost::asio::ssl::context> m_pSSLContext;
    SSLServerInfo m_SSLServerInfo;
};
...
public:
    TCPClient(const char *pIPAddress, const char *pIPPort, ConnectedCallback CnBck, ReadOrWriteCallback RdBck,
        ...
        const unsigned int uiSSLEnabled = false);

    TCPClient(const char *pIPAddress, const char *pIPPort, const unsigned int uiSSLEnabled = false);

    TCPClient(const char *pIPAddress, const char *pIPPort, boost::asio::io_service &IOService, 
        boost::asio::io_service::work &IOWork, const unsigned int uiSSLEnabled = false);
    ...
    void SetSSLClientInfo(const SSLClientInfo &sslinfo)
    {
        m_SSLClientInfo = sslinfo;
    }
...
private:
    ...
    void HandleSSLShakehand(boost::shared_ptr<TCPSessionOfClient> session_ptr,  const boost::system::error_code& error);
...
public:
    ...
    boost::shared_ptr<TCPClient> CreateTCPClient(const char *pIPAddress, const char *pIPPort, const unsigned int uiSSLEnabled = false);
...
public:
    ...
    void Begin(const bool blRunRightNow = false);

    void End();

    void SetTimeOutBase(const bool IsSecondBase);

    void Run(const bool blRunRightNow = false);
