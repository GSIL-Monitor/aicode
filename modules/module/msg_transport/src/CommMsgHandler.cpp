...
const std::string & CommMsgHandler::GetSessionID() const
{
...
int CommMsgHandler::StartReceiveInfinite()
{
    while (true)
    {
        m_nTimeout = 0; //确保read没有超时时间

        Packet pkt;
        int errcode = SyncRead(pkt);
        if (errcode != SUCCEED)
        {
            return errcode;
        }

        errcode = m_RspInifinite(pkt);
        if (errcode != SUCCEED)
        {
            return errcode;
        }

        errcode = m_ReqInifinite(boost::bind(&CommMsgHandler::SyncWrite, this, (m_strSessionID.empty() ? m_strID : m_strSessionID), _1, _2, _3, _4));
...
void CommMsgHandler::SetReqAndRspInfinite(FuncRequest req, FuncReply rsp)
{
    m_ReqInifinite = req;
    m_RspInifinite = rsp;
}
...
int CommMsgHandler::SyncRead(Packet &pt)
{
    ...
    while (m_ReadBuff.empty())
    {
        ...
        pt.strSrcID = pLocalValue->strSrcID;
    }
...
void CommMsgHandler::ReadCB(const boost::system::error_code& e, std::list<ClientMsg> *pClientMsgList, void* pLocalValue)
{
    ...
    for (auto msg = pClientMsgList->begin(); msg != pClientMsgList->end(); ++msg)
    {
        ...
        pCBValue->strSrcID = msg->strSrcID;
    }
