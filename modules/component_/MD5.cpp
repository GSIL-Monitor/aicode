...
#define II(a, b, c, d, x, s, ac)  (a) += I ((b), (c), (d)) + (x) + (unsigned int)(ac); (a) = ROTATE_LEFT ((a), (s)); (a) += (b)
    ...
    void MD5_Finish(unsigned char digest[16], MD5_CTX * context)
    {
...
#pragma warning( disable : 4996 )
    ...
    char* MD5_sign(const unsigned char *str, unsigned int len, int isBCD)
    {
        int i;
        MD5_CTX         md5;
        static char md5_str[MD5STR_LEN + 1];
        unsigned char hash[16], tmp[3];
        md5_str[0] = 0;
        ...
        if (isBCD != 0)
            for (i = 0; i < 16; i++)
            {
                _itoa((unsigned char)hash[i], (char*)tmp, 16);
                if (tmp[1] == 0)
                {
                    tmp[2] = 0; tmp[1] = tmp[0]; tmp[0] = '0';
                }
                strcat(md5_str, (char*)tmp);
            }
        else
        {
            memcpy(md5_str, hash, 16);
            md5_str[16] = '\0';
        }
...
#define MD5_STEP_SIZE (1024 * 16)
    int  MD5_File(const char* filename, unsigned char md[16])
    {
        MD5_CTX ctx;
        FILE *fp = NULL;
        int len = MD5_STEP_SIZE;
        ...
        do {
            if ((len = fread(buf, 1, MD5_STEP_SIZE, fp)) > 0) {
                MD5_Update(&ctx, buf, len);
            }
        } while (len == MD5_STEP_SIZE);
        MD5_Finish(md, &ctx);
        free(buf);
        fclose(fp);
        return 1;
    ...
    int MD5_File_V2(const char* filename, char str[32])
    {
        unsigned char md[16] = { 0 };
        if (!MD5_File(filename, md)) {
            return 0;
        }
        BCD2ASCII(md, sizeof(md), (unsigned char*)str);
        return 1;
    }

    int  MD5_Buffer(const void* buf, int len, unsigned char md[16])
    {
        MD5_CTX ctx;
        int i = 0, steplen = MD5_STEP_SIZE;

        MD5_Init(&ctx);
        for (i = 0; i < len; i += MD5_STEP_SIZE) {
            steplen = len - i > MD5_STEP_SIZE ? MD5_STEP_SIZE : len - i;
            MD5_Update(&ctx, (const unsigned char*)(buf)+i, steplen); //modi by guojt
        }
        MD5_Finish(md, &ctx);
        return 1;
    }

    int  MD5_Buffer_V2(const void* buf, int len, char str[32])
    {
        unsigned char md[16] = { 0 };
        if (!MD5_Buffer(buf, len, md)) {
            return 0;
        }
        BCD2ASCII(md, sizeof(md), (unsigned char*)str);
        return 1;
