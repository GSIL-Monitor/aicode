...
TCPSessionOfServer::TCPSessionOfServer(boost::asio::io_service &TMIOService, boost::asio::io_service& io_service,
    ServerReadOrWriteCallback RdCB, ServerReadOrWriteCallback WtCB, const unsigned int uiSSLEnabled, boost::shared_ptr<boost::asio::ssl::context> pSSLContext) :
    m_TMIOService(TMIOService), m_Socket(io_service), m_RdCB(RdCB), m_WtCB(WtCB), m_RdTcb(NULL), m_WtTcb(NULL), m_uiSSLEnabled(uiSSLEnabled)
{
    if (m_uiSSLEnabled && (NULL != pSSLContext.get()))
    {
        m_pSSLSocket.reset(new boost::asio::ssl::stream<boost::asio::ip::tcp::socket>(io_service, *pSSLContext));
    }
}

boost::asio::ip::tcp::socket::lowest_layer_type& TCPSessionOfServer::GetSocket()
{
    if (m_uiSSLEnabled && (NULL != m_pSSLSocket.get()))
    {
        return m_pSSLSocket->lowest_layer();
    }
    else
    {
...
boost::shared_ptr<boost::asio::ssl::stream<boost::asio::ip::tcp::socket> > & TCPSessionOfServer::GetSSLSocket()
{
    return m_pSSLSocket;
}
    ...
    void *pValue)
{
    ...
    if (m_uiSSLEnabled)
    {
        boost::asio::async_write((*m_pSSLSocket), boost::asio::buffer(pInputBuffer, uiBufferSize),
    ...
    void *pValue, const boost::uint32_t uiNeedReadSize)
{
    ...
    if (0 == uiNeedReadSize)
    {
        if (m_uiSSLEnabled)
        {
            (*m_pSSLSocket).async_read_some(boost::asio::buffer(pOutputBuffer, uiBufferSize),
    ...
    else
    {
        if (m_uiSSLEnabled)
        {
            boost::asio::async_read((*m_pSSLSocket), boost::asio::buffer(pOutputBuffer, uiNeedReadSize),
                boost::bind(&TCPSessionOfServer::HandleRead, shared_from_this(), pTimer,
                boost::asio::placeholders::error,
                boost::asio::placeholders::bytes_transferred, pValue));
        }
    ...
    boost::uint32_t &uiSizeofWrited, std::string &strErrMsg)
{
    ...
    if (m_uiSSLEnabled)
    {
        uiSizeofWrited = boost::asio::write((*m_pSSLSocket), boost::asio::buffer(pInputBuffer, uiBufferSize), ec);
    }
    else
    {
...
TCPServer::TCPServer(const unsigned short port, const unsigned int uiSSLEnabled) : m_Acceptor(m_IOServiceAcceptor, tcp::endpoint(tcp::v4(), port)),
m_IOWork(m_IOService), m_IOWorkAcceptor(m_IOServiceAcceptor), m_AcceptedCallback(NULL), m_SessionNum(0),
m_TimeIOWork(m_TimeIOService), m_uiSSLEnabled(uiSSLEnabled)
{
	//StartAccept();
    if (m_uiSSLEnabled)
    {
        boost::filesystem::path SSLCert = boost::filesystem::current_path() / "server.crt";
        boost::filesystem::path SSLCertKey = boost::filesystem::current_path() / "server.key";
        boost::filesystem::path SSLCertDh = boost::filesystem::current_path() / "dh1024.pem";

        m_pSSLContext.reset(new boost::asio::ssl::context(boost::asio::ssl::context::sslv23));
        m_pSSLContext->set_options(
            boost::asio::ssl::context::default_workarounds
            | boost::asio::ssl::context::no_sslv2
            | boost::asio::ssl::context::single_dh_use);
        m_pSSLContext->set_password_callback(boost::bind(&TCPServer::GetPassword, this));
        m_pSSLContext->use_certificate_chain_file(m_SSLServerInfo.strSSLCert.empty() ? SSLCert.string() : m_SSLServerInfo.strSSLCert);
        m_pSSLContext->use_private_key_file(m_SSLServerInfo.strSSLCertKey.empty() ? SSLCertKey.string() : m_SSLServerInfo.strSSLCertKey, boost::asio::ssl::context::pem);
        m_pSSLContext->use_tmp_dh_file(m_SSLServerInfo.strSSLCertDh.empty() ? SSLCertDh.string() : m_SSLServerInfo.strSSLCertDh);
    }
}
...
void TCPServer::StartAccept(ServerReadOrWriteCallback RdBck, ServerReadOrWriteCallback WtBck)
{
    ...
    boost::shared_ptr<TCPSessionOfServer> pSession(
        ...
        RdBck, WtBck, m_uiSSLEnabled, m_pSSLContext));
	...
	ServerReadOrWriteCallback RdBck, ServerReadOrWriteCallback WtBck)
{
    ...
    if (m_uiSSLEnabled)
    {
        pSession->GetSSLSocket()->async_handshake(boost::asio::ssl::stream_base::server,
            boost::bind(&TCPServer::HandleSSLShakehand, this, pSession, boost::asio::placeholders::error));
...
void TCPServer::HandleSSLShakehand(boost::shared_ptr<TCPSessionOfServer> pSession, const boost::system::error_code& error)
{
    if (error)
    {
        std::cout << "HandleSSLShakehand is error, msg is " << error.message().c_str() << std::endl;
        return;
    }
...
TCPSessionOfClient::TCPSessionOfClient(boost::asio::io_service &TMIOService,
    ...
    const unsigned int uiSSLEnabled, boost::shared_ptr<boost::asio::ssl::context> pSSLContext) :
    m_TMIOService(TMIOService),
    m_Socket(io_service), m_RdBck(RdBck), m_WtBck(WtBck),
    m_uiRdTimeOutSec(uiRdTimeOutSec), m_uiWtTimeOutSec(uiWtTimeOutSec), m_uiSSLEnabled(uiSSLEnabled)

{
    if (m_uiSSLEnabled && (NULL != pSSLContext.get()))
    {
        m_pSSLSocket.reset(new boost::asio::ssl::stream<boost::asio::ip::tcp::socket>(io_service, *pSSLContext));
        m_pSSLSocket->set_verify_mode(boost::asio::ssl::verify_peer);
        m_pSSLSocket->set_verify_callback(
            boost::bind(&TCPSessionOfClient::VerifyCertificate, this, _1, _2));
    }
}

boost::asio::ip::tcp::socket::lowest_layer_type& TCPSessionOfClient::GetSocket()
{
    if (m_uiSSLEnabled && (NULL != m_pSSLSocket.get()))
    {
        return m_pSSLSocket->lowest_layer();
...
boost::shared_ptr<boost::asio::ssl::stream<boost::asio::ip::tcp::socket> > & TCPSessionOfClient::GetSSLSocket()
{
    return m_pSSLSocket;
}
...
bool TCPSessionOfClient::VerifyCertificate(bool preverified, boost::asio::ssl::verify_context& ctx)
{
...
bool TCPSessionOfClient::SyncRead(char *pOutputBuffer, const boost::uint32_t uiBufferSize, boost::uint32_t &uiSizeofReaded, std::string &strErrMsg)
{
    ...
    if (m_uiSSLEnabled)
    {
        uiSizeofReaded = boost::asio::read((*m_pSSLSocket), boost::asio::buffer(pOutputBuffer, uiBufferSize), ec);
    }
    else
    {
...
TCPClient::TCPClient(const char *pIPAddress, const char *pIPPort, ConnectedCallback CnBck, ReadOrWriteCallback RdBck,
    ...
    const boost::uint32_t uiRdTimeOutSec,
    const boost::uint32_t uiWtTimeOutSec,
    const unsigned int uiSSLEnabled) : m_strIPAddress(pIPAddress), m_strIPPort(pIPPort),
                                                                    ...
                                                                    m_uiSSLEnabled(uiSSLEnabled)
{
    if (m_uiSSLEnabled)
    {
        boost::filesystem::path SSLCert = boost::filesystem::current_path() / "server.crt";
        m_pSSLContext.reset(new boost::asio::ssl::context(boost::asio::ssl::context::sslv23));
        m_pSSLContext->load_verify_file(m_SSLClientInfo.strSSLCert.empty() ? SSLCert.string() : m_SSLClientInfo.strSSLCert);

    }

    m_Session.reset(new TCPSessionOfClient(m_TimeIOService, m_IOService, RdBck, WtBck, uiRdTimeOutSec, uiWtTimeOutSec, m_uiSSLEnabled, m_pSSLContext));
    AsyncConnect(uiCnTimeOutSec);

}

TCPClient::TCPClient(const char *pIPAddress, const char *pIPPort, const unsigned int uiSSLEnabled) : m_strIPAddress(pIPAddress), m_strIPPort(pIPPort),
                                                        m_CnBck(NULL),
...
TCPClient::TCPClient(const char *pIPAddress, const char *pIPPort, 
                                                        ...
                                                        const unsigned int uiSSLEnabled) :
...
void TCPClient::HandleSSLShakehand(boost::shared_ptr<TCPSessionOfClient> session_ptr, const boost::system::error_code& error)
{
    if (error)
    {
        std::cout << "HandleSSLShakehand is error, msg is " << error.message().c_str() << std::endl;
    ...
    const boost::uint32_t uiWtTimeOutSec)
{
    ...
    m_uiRdTimeOutSec = uiRdTimeOutSec;
    m_uiWtTimeOutSec = uiWtTimeOutSec;
    printf("m_uiSSLEnabled=%u, m_pSSLContext is %s\n", m_uiSSLEnabled, NULL == m_pSSLContext.get() ? "null " : "not null ");
    m_Session.reset(new TCPSessionOfClient(m_TimeIOService, m_IOService, RdBck, WtBck, uiRdTimeOutSec, uiWtTimeOutSec, m_uiSSLEnabled, m_pSSLContext));
...
void TCPClient::HandleConnect(boost::shared_ptr<TCPSessionOfClient> session_ptr, const boost::system::error_code& ec)
{
    ...
    if (m_uiSSLEnabled)
    {
        session_ptr->GetSSLSocket()->async_handshake(boost::asio::ssl::stream_base::client,
            boost::bind(&TCPClient::HandleSSLShakehand, this, session_ptr, boost::asio::placeholders::error));
    ...
    const boost::uint32_t uiWtTimeOutSec /*= 0*/)
{
    ...
    m_Session.reset(new TCPSessionOfClient(m_TimeIOService, m_IOService, RdBck, WtBck, uiRdTimeOutSec, 
        uiWtTimeOutSec, m_uiSSLEnabled, m_pSSLContext));
...
void TimeOutHandler::Begin(const bool blRunRightNow)
{
    ...
    if (m_IsSecondBase)
    {
        blRunRightNow ? m_Timer.expires_from_now(boost::posix_time::milliseconds(500), ecn) :
...
void TimeOutHandler::Run(const bool blRunRightNow)
{
    if (!m_uiSeparateModel)
    {
        return;
...
boost::shared_ptr<TCPClient> TCPClientEx::CreateTCPClient(const char *pIPAddress, const char *pIPPort, const unsigned int uiSSLEnabled)
{
    return boost::shared_ptr<TCPClient>(new TCPClient(pIPAddress, pIPPort, m_IOService, m_IOWork, uiSSLEnabled));
}
